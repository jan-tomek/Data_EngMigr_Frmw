...
FOR %%i IN (.\1_IN\*.xlsx) DO (
    ECHO %%i
    @py ExcelLoader.py --infile "%%i" --lookup --codetest
    ...
    FOR %%j IN (.\2_RUN\*.sql) DO (
        ...
        FINDSTR "[" ".\3_LOG\%%~nj.log" | FINDSTR -v "Cannot use DROP VIEW with" | FINDSTR -v "There is already an object named" |SORT /UNIQUE > ".\3_LOG\%%~nj.err" 2> NUL
